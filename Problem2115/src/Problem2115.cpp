 ...
 * @url https://leetcode.com/problems/find-all-possible-recipes-from-given-supplies/submissions/1584131540/
 */
class Problem2115 {
public:
    std::vector<std::string> findAllRecipes(
        ...
        std::function<bool(std::string)> dfs = [&](const std::string &item) {
            ...
            if (recipe_index.contains(item)) {
                for (const auto &ingredient: ingredients[recipe_index[item]]) {
                    if (!dfs(ingredient)) {
                        return false;
                    }
                }
...
int main() {
    ...
    const auto &result1 = solution.findAllRecipes(recipes1, ingredients1, supplies1);
    ...
    std::vector<std::vector<std::string>> ingredients2 = {{"yeast", "flour"},
                                                          {"bread", "meat"}};
    std::vector<std::string> supplies2 = {"yeast", "flour", "meat"};

    const auto result2 = solution.findAllRecipes(recipes2, ingredients2, supplies2);
    ...
    std::vector<std::vector<std::string>> ingredients3 = {{"yeast",    "flour"},
                                                          {"bread",    "meat"},
                                                          {"sandwich", "meat", "bread"}};
    std::vector<std::string> supplies3 = {"yeast", "flour", "meat"};

    const auto result3 = solution.findAllRecipes(recipes3, ingredients3, supplies3);
